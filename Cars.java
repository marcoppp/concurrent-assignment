...
class Ground extends JPanel {
    ...
    public Ground(Cars  c, Simulator<CarModel> sim) {
        ...
        // Use periodic repaint of whole playground at fps frames-per-second to eliminate platform differences
    ...
    // The following internal graphical methods are only called via the GUI-thread
    ...
    public void paintComponent(Graphics g) {
        ...
        sim.applyToAll((car) -> { paintCar(field, car); });
...
class ControlPanel extends JPanel {
    ...
    JCheckBox slow   = new JCheckBox("Slow", false);
    ...
    public ControlPanel (Cars c) {
        ...
        start_all.addActionListener( (event) -> { cars.startAll(); } );

        JButton stop_all = new JButton("Stop all");
        stop_all.setMargin(bmargin);
        stop_all.addActionListener( (event) -> { cars.stopAll(); } );

        keep.addItemListener( (event) -> { cars.setKeep(keep.isSelected()); });
        slow.addItemListener( (event) -> { cars.setSlow(slow.isSelected()); });
        ...
        barrier_on.addActionListener( (event) -> { cars.barrierOn(); });

        barrier_off.addActionListener( (event) -> { cars.barrierOff(); });
...
public class Cars extends JFrame implements CarDisplayI {
    ...
    class SetThread extends Thread {
        ...
        public void run() {
            try {
                ...
                EventQueue.invokeLater( () -> { barrierSetDone(); });
    ...
    public static void main(String [] args) {
        try {
            ...
            Thread.sleep(100*365*24*60*60*1000); // Sleep here for a hundred years
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
    ...
    public void setFreeze(boolean freeze) {
        if (freeze) sim.pause(); else sim.resume();
    }
    ...
    public Pos getBarrierPos(int no) {    // Identify pos. right before barrier line
...
class CarTestWrapper implements CarTestingI {
    ...
    public void setSlow(final boolean slowdown) {
        EventQueue.invokeLater( () -> { cars.setSlow(slowdown); } );
    }

    public void removeCar(final int no) {
        EventQueue.invokeLater( () -> { cars.removeCar(no); } );
    }

    public void restoreCar(final int no) {
        EventQueue.invokeLater( () -> { cars.restoreCar(no); } );
    }

    public void setSpeed(final int no, final double speed) {
        EventQueue.invokeLater( () ->  { cars.setSpeed(no, speed); } );
    }

    public void setVariation(final int no, final int var) {
        EventQueue.invokeLater( () -> { cars.setVariation(no, var); } );
    }
...
interface ClockI { public long getTime(); }
...
class Clock implements ClockI {
    ...
    synchronized public void setRate(double r) {
        if (r < 0.0) throw new IllegalArgumentException("Negative Clock rate");
...
class CarModel implements CarI, Tickable {
    ...
    /*
     * CarModel models the movement of a car along a route of waypoints (destinations)
     * 
     * The movement is driven by calls of tick() from the virtual clock provided.
     * 
     */
    ...
    void extendTarget () {
        while (target.next != null) {
            if (!sameDir(target.dir, target.next.dir)) break;
            target = target.next;
        }
    }
    ...
    synchronized public void awaitArrivalAt(long id) throws InterruptedException {
        ...
        while (lastArrival < id) {	wait(); }
    }

    void arriveAt(Waypoint wp) { 
        lastArrival = wp.id;
        if (wp.cb != null) exec.submit(wp.cb);
        notifyAll();
    }
    ...
    boolean starting(long l) {
        return Location.dist(l,Location.location(start.pos)) <= startDist;
    }

    double arrivalCap() {
        return Math.max(1.0, Math.ceil(Location.dist(loc,target.loc)*8)) * 5.0;
    }


    double departureCap() {
        return Math.max(1.0, Math.ceil(Location.dist(start.loc,loc)*10)) * 5.0;
    }

    boolean isSlow() {
        return starting(loc) || approachingTarget(loc) || 
...
interface Procedure<T> { void apply(T t); }   

interface Tickable { public void tick(); public void start(); public void stop(); }
...
class Location {
    ...
    /*
     * Locations are fine-grained positions within the playground 
     * represented as fixed decimal values of the Pos coordinate system.
     * (I.e. the x-axis points downwards and the y axis to the right.)
     * 
     * Locations are encoded as two ints within a long for efficient
     * communication to the GUI.
     * 
     * This class provides various conversion functions for this encoding
     * as well as some vector operations.
     */
    ...
    // (Kind of) unit-vector
    static long direction(long l) {
        return scale(l, 1.0/ len(l));
    }

    // Manhattan length of vector
    static double len(long l) {
        return ((double) Math.abs(locX(l))) / resolution + 
                ((double) Math.abs(locY(l))) / resolution;
    }
...
class Layout {
    ...
    public static Pos canonical(Pos pos) {
        ...
        } catch (Exception e) { throw new IllegalArgumentException("Position out of grounds: " + pos); }
    }
    ...
    static {
        ...
        // Alley (downwards)
        for (int i = 0; i <= ROWS-2; i++) { tracksCCW[i][0] = getPos(i+1, 0); }
        // Private lanes 
        for (int no = 5; no <= 8; no++) {
            for (int i = 1; i <= ROWS-1; i++) { tracksCCW[i][no+3] = getPos(i-1, no+3); }
        }
        // Upper lane 
        for (int j = 1; j <= COLS-1; j++) { tracksCCW[0][j]  = getPos(0, j-1); }
        // Lower lane (without shed)
        for (int j = 0; j <= COLS-2; j++) { tracksCCW[ROWS-1][j] = getPos(ROWS-1, j+1); }

        // Shed avoidance
        tracksCCW[9][0] = getPos(9,1);
        tracksCCW[9][1] = getPos(9,2);
        tracksCCW[9][2] = getPos(10,2);

        // CCW branching points
        for (int no = 5; no <= 8; no++) { branchCCW[no-5] = getPos(ROWS-2, no+3); }

        // Define next position of all clockwise tracks (except branching points) 

        // Toddler's special path
        tracksCW[5][2] = getPos(4,2);
        tracksCW[4][2] = getPos(4,3);
        tracksCW[4][3] = getPos(5,3);
        tracksCW[5][3] = getPos(5,2);

        // Alley (upwards)
        for (int i = 2; i <= ROWS-2; i++) { tracksCW[i][0] = getPos(i-1, 0); }
        // Private lanes (some parts overwritten below)
        for (int no = 1; no <= 4; no++) {
            for (int i = 1; i <= ROWS-3; i++) { tracksCW[i][no+3] = getPos(i+1, no+3); }
        }
        // Lower lane (car nos. 3,4)
        for (int j = 1; j <= 7; j++) { tracksCW[ROWS-2][j] = getPos(ROWS-2, j-1); }
        // Lower lane (car nos. 1,2)
        for (int j = 1; j <= 5; j++) { tracksCW[ROWS-3][j] = getPos(ROWS-3, j-1); }
        // Upper lane
        for (int j = 0; j <= 6; j++) { tracksCW[1][j] = getPos(1, j+1); }

        // CCW branching points
        for (int no = 1; no <= 4; no++) { branchCW[no-1] = getPos(2, no+3); }
    }

    public static boolean isShedPos(Pos p) {
        return (p.row==ROWS-1 && p.col <= 1);  // Lower left corner
    }

    public static boolean isHutPos(Pos p) {
        if     (p.row < 2 || p.row > 7 || p.col < 1 || p.col > 3) return false;
        return (p.row < 4 || p.row > 5 || p.col < 2);
    }
    ...
    public static boolean isSlowPos(Pos p) {
        return (p.col ==0  && p.row <= ROWS-1);
    }

    public static Pos getStartPos(int no) {
        if (no == 0) return getPos(5,2);
        if (no  < 5) return getPos(upperBarRow-1, 3 + no);
        return getPos(lowerBarRow+1, 3 + no);
    }

    public static Pos getBarrierPos(int no) {
        return getPos( no < 5 ? upperBarRow : lowerBarRow, 3 + no);
    }
    ...
    public static Pos nextPos(int no, Pos pos) {
        ...
        if (no < 5) {
            ...
            if (pos.row == 1 && pos.col == mycol) {
                // Branching point 
                return branchCW[no - 1];
            } else {
                return tracksCW[pos.row][pos.col];
            }
        } else {
            // CCW
            if (pos.row == ROWS-1 && pos.col == mycol) {
                // Branching point 
                return branchCCW[no - 5];
            } else {
